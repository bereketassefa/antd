...
// console.log(decodedName)
  ...
  const hadleNavigateProfile = async (e) => {
    e.preventDefault();
    try {
      const url = `${import.meta.env.VITE_FIND_MY_DATA}/${DataProducts.accountId}`;
      await axios.get(url);
  ...
  return (
    <div className="dark:bg-[#1b1f23] max-w-[780px] md:w-[780px] bg-white drop-shadow-xl">
      ...
      {(!hasSliderTouched ? DataProducts : DataProducts?.filter(item => item?.ProductPrice <= inputValue))?.map((item) => {

        return (
          <div className="border-2 rounded-lg my-4">
            <div className="flex justify-end">
              <p className="dark:text-white">Price</p>
              <div dir="rtl" className="bg-gray-300 w-36 h-10 flex gap-2 items-center rounded-bl-[40px]">
                <p>ETB</p>
            ...
            <div className="flex flex-col md:flex-row justify-between md:items-center px-4 ">
              <div className="justify-start flex flex-col lg:justify-center gap-2  mt-4">
                <div className="flex  justify-start items-center">
                  <Avatar onClick={hadleNavigateProfile} img={item.profilePicture? item.profilePicture : alternativeProfile} />
                  <div>
                    <h2 onClick={hadleNavigateProfile} className="dark:text-white font-bold text-[#000]  text-[13px]   mx-1">
                      {item.comapanyName.toLowerCase()}
                    </h2>
                    <p className="dark:text-white pl-3 flex font-bold ">{item.productName}</p>
                  </div>
                </div>
                <div className="flex justify-between">
                  <p className="dark:text-white max-w-[670px] md:w-[670px] mb-6">
      ...
      <a href="" className="flex flex-col justify-center items-center pb-8">
        <p className="dark:text-white flex justify-center text-[#3222C6]">See More</p>
